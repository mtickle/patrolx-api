...
router.post("/postRoadIncident", async (req, res) => {
    ...
    const internal_id = req.body.id;
    const latitude = req.body.latitude;
    const longitude = req.body.longitude;
    const commonname = req.body.commonName;
    const reason = req.body.reason;
    const condition_name = req.body.condition;
    ...
    const query = `CALL public.add_roadincident('${internal_id}','${latitude}','${longitude}','${commonname}','${reason}','${condition_name}','${incidenttype}','${severity}','${direction}','${location_name}','${countyname}','${city}','${start_date}','${end_date}','${lastupdate}','${road}','${isdetour}','${detour}','${lanesclosed}');`
...
router.get('/getAllRoadIncidents', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM get_roadincidents($1)', [recordLimit]);
        res.json(result.rows);
    } catch (error) {
        console.error('Error get_roadincidents:', error);
